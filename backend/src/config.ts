...
export const config = {
  ...
  searchMaxResults: toInt(process.env.SEARCH_MAX_RESULTS, 10),
  searchCountry: process.env.SEARCH_COUNTRY ?? 'cz',
  searchLanguage: process.env.SEARCH_LANGUAGE ?? 'cs',
  searchRateLimitMs: toInt(process.env.SEARCH_RATE_LIMIT_MS, 250),
  searchRetryCount: toInt(process.env.SEARCH_RETRY_COUNT, 1),
  ...
  maxSearchQueriesPerCompany: toInt(process.env.MAX_SEARCH_QUERIES_PER_COMPANY ?? process.env.WEBSITE_SEARCH_MAX_QUERIES, 8),
